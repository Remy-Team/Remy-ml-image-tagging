"""Simple Streamlit web-ui for tagging service"""
...
IMAGE_TAGGING_ENDPOINT = os.getenv(
    "IMAGE_TAGGING_ENDPOINT", "http://localhost:3000/predict"
)
...
def infer_image(
    ...
    extension: str = "jpeg",
    endpoint_url: str = IMAGE_TAGGING_ENDPOINT,
):
    """Sends an image for tags inference
    and writes the result to 'tagging_result' if
    successfull"""
    if extension == "jpg":
        extension = "jpeg"
    ...
    files = {
        "imgs": (
            f"inference_image.{extension}",
            image_bytes_stream,
            f"image/{extension}",
        )
    }
    response = requests.post(endpoint_url, headers=headers, files=files, timeout=300)
    if response.status_code != 200:
        st.error(
            f"Server has failed to process request."
            f"Status code: {response.status_code}. Response content: {response.content}"
        )
        return
...
def main_page():
    ...
    send_to_inference_condition = (
        btn and image and not st.session_state.get("tagging_result")
    )
